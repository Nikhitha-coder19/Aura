...
class SafetyService {
     ...
     */
    checkSafety(request, intent, riskAssessment) {
        const warnings = [];
        // Check if already blocked by risk assessment
        if (riskAssessment.level === types_1.RiskLevel.BLOCKED) {
            return {
                passed: false,
                blocked: true,
        ...
        if (intent === types_1.Intent.VISION_ANALYZE && !request.imageBase64) {
            return {
                passed: false,
                blocked: false,
        ...
        if ((intent === types_1.Intent.SEND_MESSAGE || intent === types_1.Intent.DRAFT_EMAIL) && !request.confirmed) {
     ...
     */
    validateEntities(intent, request) {
        const warnings = [];
        switch (intent) {
     ...
     */
    canAutoExecute(riskLevel, confirmed) {
        if (riskLevel === types_1.RiskLevel.BLOCKED) {
            return false;
        }
     ...
     */
    generateSafetyStep(result, stepNumber) {
        if (result.blocked) {
            return {
                step: stepNumber,
                label: 'Safety check',
